...
- 基于粘贴文字或共享脚本规划三个创作方向，并并行生成三篇可编辑的新脚本。
- 用户之间的任务和字幕严格隔离。
...
SRT_SCRIPT_FISSION_TIMEOUT_SECONDS=180
```
...
拥有“脚本裂变”权限的账号可访问 `/script-fission`。用户可以直接粘贴不超过
30,000 字的来源脚本；同时拥有“共享脚本库”权限时，也可以搜索并选择团队脚本。
系统先规划三个不同的切入角度、开场钩子和叙事结构，再并行生成三篇完整文字
脚本。单个版本失败不会影响其他结果，并可独立重试。

裂变结果可以在浏览器中编辑和复制，只有用户主动点击保存时才会写入共享脚本
库。来源、规划和生成结果本身不保存生成历史，页面刷新后消失。脚本文字会发送
到所配置的第三方模型服务；默认单次规划或生成调用超时为 180 秒。
...
- 主站会记录页面路径、访问时间、IP 和离线解析得到的近似归属地。成功登录后，
  当前 IP 会与账号建立多对多关联；登录前的匿名访问不会追溯到账号。
- 关键业务操作与安全事件会记录成功/失败、账号、IP、资源 ID 和不含正文的数值型
  元数据。密码、Cookie、Token、脚本正文、字幕正文、违禁词内容和下载地址不会进入
  埋点。
- 页面访问、操作明细和原始 IP 关联默认保留 90 天，之后只保留按日、用户和操作类型
  聚合的数量。可通过 `SRT_ANALYTICS_RETENTION_DAYS` 调整明细保留天数。
- IP 归属地来自随容器构建的 ip2region IPv4/IPv6 离线库，只能代表近似城市和
  运营商，不能用于识别街道、住址或个人。
